...
from xiaoIceServer.xiaoice import chat,chatWithImg
...
class ChatHandler(BaseHandler):
    ...
    def accessibility(self):
        ...
        correct_auth = [item.replace('\r', '').replace('\n', '')
                        for item in open(os.path.join(os.getcwd(),'xiaoIceServer','key.txt'), encoding='u8').readlines()]
    ...
    def run_request(self):
        ...
        if user_input and user_input_type:
            try:
