...
def __read_headers():
    ...
    real = {}
    f = open(os.path.join(os.getcwd(),'xiaoIceServer','headers.txt'), encoding='utf-8')
...
def __renew_headers():
    ...
    # write files
    with open(os.path.join(os.getcwd(),'xiaoIceServer','headers.txt'), 'r')as f:
        text = f.read()
        text = text.replace(old_csrf, new_csrf)
    with open(os.path.join(os.getcwd(),'xiaoIceServer','headers.txt'), 'w') as f:
        f.write(text)
    return old_headers
...
# 根据时间戳决定要获取哪些记录，countn休眠后提取
def __getResponse(sendingTimeStamp:int,countn:int) -> str:
    # get response
    cur_headers = __read_headers()
    time.sleep(countn)
    polling_count = 0
    ...
    while 1:
        ...
        for i in range(len(messages)):
            ...
            #print('r --->'+str(receivedTimeStamp))
            #print('sender -> '+str(messages[i]['sender_id']))
